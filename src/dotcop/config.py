...
class Config: 
    ...
    def test_configfile(self):
        ...
        if XDG_CONFIG_HOME == None or not Path(XDG_CONFIG_HOME).is_dir(): 
        ...
        if not configdir_path.is_dir(): 
            ...
            try: 
                ...
                self.logger.critical(f"Configuration directory \"dotcop\" could not be created in {XDG_CONFIG_HOME}. This could indicate incorrect access right. This could indicate incorrect access rights.")
                raise

        # Test if XDG_CONFIG_HOME/dotcop/dotcop.yml exists, if not copy default configuration from project root to destination
